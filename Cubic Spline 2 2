...
def min_max_analyzer(x: np.array, y: np.array) -> MinMaxProcessing:
    ...
    while i < len(k):
        ...
        i += 1
    print('x_min_array_start', len(x_min_array))
    print('x_max_array_start', len(x_max_array))
    ...
    # Цикл для удаления точек минимума, если расстояние между ними менее 10 точек.
    # Сравнивается текущее значение с предыдущим
    i = 2
    ...
    while i < (len(y_max_array)):
        if y_max_array[i]-y_max_array[i-1]> (y_max_array[i]/2) :
            ...
            x_max_array.remove(x_max_array[i-1])
        i+=1
    # Цикл для удаления точек максимума, если между ними разница по игреку больше половины 1/4 разницы общего максимума к общему минимуму по игреку.
    # Нулевое значение ноль, поэтому отсчёт с 2-го элемента
    i = 2
    while i < (len(y_min_array)):
    ...
    print('x_min_array', len(x_min_array))
    print('x_max_array', len(x_max_array))
    ...
    print('y_max_array_line', len(y_max_array_line))
    ...
    while i < (len(x_min_array_line)):
        ...
        i += 1
    print('x_middle_line_array', len(x_middle_line_array))
    def surch_distance_min_max(x_middle_line_array: list, y_middle_line_array: list, x_min_array: list,
        ...
        while i < (len(y_max_array) - 1):
            k = (x_max_array_line.index(x_max_array[i])-10)
            while k < (x_max_array_line.index(x_max_array[i])+10):
